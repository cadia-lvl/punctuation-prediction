...
for p in 5 10 15 20; do
sbatch \
--job-name=${input}-fairseq-translate-${p}wer \
--mem 12G --time 0-08:00 --gres=gpu:1 \
--output=$wertestout/wer${p}_punctuate.log \
--wrap="srun \
python fairseq-punctuate.py \
$modeldir $bindatadir $wer_test/../${input}${ext}.tokenized/code \
$toktmp/wer${p}perc.clean.$src \
