...
srun python ${scriptdir}/process/preprocess_en_lower.py \
ep_cleaned.txt ep_cleaned_formatted.txt \
&>logs/preprocess_data.log

echo "split it up into train, dev and test sets"
head -n -80000 ep_cleaned_formatted.txt > ep.train.txt
tail -n 80000 ep_cleaned_formatted.txt > devtest
head -n -40000 devtest > ep.dev.txt
tail -n 40000 devtest > ep.test.txt
echo "Cleaning up..."
rm -f devtest
...
for f in ep.*.txt ; do grep -o .PERIOD $f | wc -l; done
for f in ep.*.txt ; do grep -o ,COMMA $f | wc -l; done
for f in ep.*.txt ; do grep -o ?QUESTIONMARK $f | wc -l; done
for f in ep.*.txt ; do grep -o \!EXCLAMATIONMARK $f | wc -l; done
for f in ep.*.txt ; do grep -o :COLON $f | wc -l; done
for f in ep.*.txt ; do grep -o \;SEMICOLON $f | wc -l; done
for f in ep.*.txt ; do grep -o "\-DASH" $f | wc -l; done
