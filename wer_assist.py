...
def apply_wer(nperc, wordList=processed_words[:int(0.2*len(processed_words))], 
    randomWords=unique_words, punctuations=punctList):
    scoreList = []
    ...
    wordListFinish = [list(filter(lambda x: x not in ['DELETION'], sublist)) for sublist in wordList]
